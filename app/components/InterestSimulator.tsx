...
export default function InterestSimulator({
  ...
  const referenceScenarios = useMemo(() => {
    ...
    const longTerm =
      longHorizon > horizon
        ? RISK_PROFILES.filter(
            (p) => p.label === "Modéré" || p.label === "Optimiste",
          ).map((p) => ({
            ...p,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        <div className="bg-surface-deep/50 p-4 rounded-xl border border-line/50">
          <span className="text-[10px] text-subtle font-bold uppercase tracking-widest">
            {t("dashboard.card3.capital")}
          </span>
          <p className="text-xl font-bold text-body-soft mt-1">
            {formatCUR(finalData.invested)}
          </p>
        </div>
        <div className="bg-surface-deep/50 p-4 rounded-xl border border-line/50">
          <span className="text-[10px] text-subtle font-bold uppercase tracking-widest">
            {t("dashboard.card3.interest")}
          </span>
          <p className="text-xl font-bold text-green-400 mt-1">
            +{formatCUR(finalData.interest)}
          </p>
        </div>
        <div className="bg-blue-600/10 p-4 rounded-xl border border-blue-500/20">
          <span className="text-[10px] text-blue-400 font-bold uppercase tracking-widest">
            {t("dashboard.card3.finalCapital")}
          </span>
          <p className="text-2xl font-black text-blue-400 mt-1">
            {formatCUR(finalData.total)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          ...
          <input
            ...
            step="100"
            value={dca}
            onChange={(e) => setDca(parseInt(e.target.value))}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <label className="text-xs font-bold text-muted uppercase tracking-widest">
              {t("dashboard.card4.annualReturn")}
            </label>
          ...
          {referenceScenarios.map((s) => (
            <div key={s.label} className={`p-4 rounded-xl border ${s.color}`}>
              <p className="text-xs font-bold uppercase tracking-widest mb-1">
                {s.emoji} {s.label} ({formatCUR(dca)}/m à {s.rate}%, {s.years}{" "}
                ans)
              </p>
